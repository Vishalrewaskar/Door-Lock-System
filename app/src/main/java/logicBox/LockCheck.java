...
public class LockCheck {
    ...
    public void checkLockFree(final String lockNumber, final String userid) {
        ...
        Log.d(TAG, "checkLockFree: querying lockentry/[" + lockNumber + "] (length=" + lockNumber.length() + ") for user " + userid);
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if (dataSnapshot.exists()) {
                    ...
                    myRef.addListenerForSingleValueEvent(new ValueEventListener() {
                                ...
                                reference.addListenerForSingleValueEvent(new ValueEventListener() {
                                    ...
                                    public void onDataChange(DataSnapshot dataSnapshot) {
                                        if (dataSnapshot.exists()) {
                                            ...
                                            if (Boolean.TRUE.equals(dataCollector.get(lockNumber))) {
                                                eventLock.eventLockFree(true, "guest", null);
                                            } else {
                                                eventLock.eventLockFree(false, null, "This lock is already registered to someone else, and you haven't been granted guest access to it.");
                                            }
                                        } else {
                                            eventLock.eventLockFree(false, null, "This lock is already registered to someone else. No guest access has been granted to your account for any lock.");
                                        }
                                    }
                        ...
                        @Override
                        public void onCancelled(DatabaseError databaseError) {
                            eventLock.eventLockFree(false, null, "Could not check lock ownership: " + databaseError.getMessage());//DB Error
                        }
                    });
                } else {
